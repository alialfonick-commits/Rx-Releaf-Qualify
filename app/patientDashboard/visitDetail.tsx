...
export default function VisitDetail ({ onNext }: { onNext: () => void }) {
    ...
    <>
      <div className='bg-[#FFFFFF] border border-[#D7DED3] rounded-xl sm:p-6 p-4 [&_strong]:font-semibold [&_strong]:text-[20px] [&_strong]:text-[#2B3B33] [&_label]:font-medium'>
        <div className='flex items-center gap-3 pb-5'>
          <Stethoscope
            color='#5E6E66'
            className='bg-[#CCD7C6] size-10 p-2 rounded-lg text-[#5E6E66]'
          />
        ...
        <div className='grid md:grid-cols-3 sm:grid-cols-2 gap-4 [&_label]:text-sm [&>div]:flex [&>div]:flex-col [&>div]:gap-2'>
          ...
          <div className='[&_span]:block! [&_span]:truncate! max-sm:w-full'>
            <div className='flex items-center gap-2'>
            ...
            >
              ...
              <SelectContent className='sm:max-w-96 max-w-80'>
                {US_STATES.map(state => (
                  <SelectItem
                    key={state.value}
                    value={state.value}
                    className='[&_span]:block! [&_span]:truncate! [&_button]:block [&_button]:truncate'
                  >
                    {state.label}
                  </SelectItem>
                ))}
          ...
          <div>
            ...
            <Input
              ...
              readOnly
              required
              className='cursor-not-allowed block! truncate! w-full'
            />
          </div>
          ...
          <div className='[&_span]:block! [&_span]:truncate! max-sm:w-full'>
            <label
              htmlFor='form-visit'
              className='sm:whitespace-nowrap flex overflow-hidden'
            >
              Urgent Care Visit: Consultation + Prescription sent to your..
            </label>
            <Select
              ...
              <SelectContent className='sm:max-w-96 max-w-80'>
                {exams.map((exam: any) => (
                  <SelectItem
                    key={exam.id}
                    value={exam.id}
                    className='[&_button]:block! [&_button]:truncate! [&_button]:w-full'
                  >
                    {exam.title}
                  </SelectItem>
                ))}
      ...
      <div className='bg-[#FFFFFF] shadow-[0px_1px_2px_0px_#0000000D] border border-[#D7DED3] rounded-xl sm:p-6 p-4 mt-2 [&_strong]:font-semibold [&_strong]:text-[20px]'>
        <div className='flex items-center gap-3 pb-5'>
          <User
            color='#5E6E66'
            className='bg-[#CCD7C6] size-10 p-2 rounded-lg'
          />
        ...
        <div className='grid grid-cols-2 gap-2 max-w-104 mx-auto w-full pt-6 [&_Button]:py-6 [&_Button]:w-full [&_Button]:text-white [&_Button]:cursor-pointer [&_button]:uppercase'>
          ...
          <Button onClick={onNext} className='bg-[#D39A05] hover:bg-[#5E6E66]'>
            Next
          </Button>
        </div>
      </div>
